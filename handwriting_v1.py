...
from transpose import convolution_transpose
...
class ConvLayer:
    ...
    def forward(self, x):
        ...
        # 把輸入圖片與過濾器轉換成矩陣
        self.cols = convolution_transpose.im2col(x, flt_h, flt_w, y_h, y_w, stride, pad)
    ...
    def backward(self, grad_y):
        ...
        self.grad_x = convolution_transpose.col2im(grad_cols.T, x_shape, flt_h, flt_w, y_h, y_w, stride, pad)
...
class PoolingLayer:
    ...
    def forward(self, x):
        ...
        # 把輸入圖片轉換成矩陣
        cols = convolution_transpose.im2col(x, pool, pool, y_h, y_w, pool, pad)
        cols = cols.T.reshape(n_bt*y_h*y_w*x_ch, pool*pool)  # taisetusna change
    ...
    def backward(self, grad_y):
        ...
        # 輸入的梯度
        x_shape = (n_bt, x_ch, x_h, x_w)
        self.grad_x = convolution_transpose.col2im(grad_cols, x_shape, pool, pool, y_h, y_w, pool, pad)
